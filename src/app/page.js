...
const PhoneMockup = ({ children, className = "" }) => (
  <div className={`relative border-8 border-gray-800 rounded-[3rem] h-[600px] w-[300px] bg-gray-800 ${className}`}>
...
const MessageBubble = ({ text, sent = false }) => (
  <div className={`max-w-[80%] rounded-2xl p-3 mb-2 ${sent ? 'ml-auto bg-primary text-white' : 'bg-gray-200 text-gray-800'}`}>
    {text}
  </div>
)
